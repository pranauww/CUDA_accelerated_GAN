...
BATCH_SIZE = 16  # Very small batch size for stability
NOISE_DIM = 100
IMG_DIM = 28 * 28
EPOCHS = 100
LR_G = 1e-4
LR_D = 1e-4  # Same learning rate
BETA1 = 0.5
BETA2 = 0.999
EPSILON = 1e-8
SAVE_EVERY = 10
...
# --- Training Loop ---
def save_images(samples, epoch, out_dir='samples_final'):
...
print('Starting final training with improved strategy...')
losses_G, losses_D = [], []
real_accs, fake_accs = [], []
...
for epoch in range(1, EPOCHS + 1):
    ...
    for batch_idx, (real_imgs, _) in enumerate(train_loader):
        ...
        # --- Compute losses with improved strategy ---
        ...
        # Train discriminator only when it's not too confident
        d_real_mean = D_real.mean()
        d_fake_mean = D_fake.mean()
        
        # Skip discriminator training if it's too confident
        skip_discriminator = (d_real_mean > 0.8) or (d_fake_mean < 0.2)
    ...
    if epoch % SAVE_EVERY == 0:
        ...
        plt.tight_layout()
        plt.savefig(f'samples_final/d_output_hist_epoch_{epoch}.png', dpi=150, bbox_inches='tight')
        plt.close()
