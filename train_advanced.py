...
LR_G = 1e-4  # Much smaller learning rates
LR_D = 5e-5  # Even smaller for discriminator
BETA1 = 0.5
BETA2 = 0.999
EPSILON = 1e-8
SAVE_EVERY = 10
...
def initialize_weights(layer):
    ...
    if isinstance(layer, LinearLayer):
        ...
        # Initialize weights
        weights = np.random.normal(0, std, (fan_in, fan_out)).astype(np.float32)
        biases = np.zeros(fan_out, dtype=np.float32)
        
        # Copy to GPU using the proper method
        weights_flat = np.ascontiguousarray(weights.ravel(order='C'))
        harness.copy_to_gpu(weights_flat, layer.W)
        harness.copy_to_gpu(biases, layer.b)

# Skip weight initialization for now to avoid the memory copy issue
# We'll use the default initialization from the layer creation
...
# --- Training Loop ---
def save_images(samples, epoch, out_dir='samples_advanced'):
...
def apply_label_smoothing(labels, smoothing=0.1, is_real=True):
    """Apply label smoothing correctly"""
    if is_real:
        return labels * (1 - smoothing) + 0.5 * smoothing
    else:
        return labels * (1 - smoothing) + 0.5 * smoothing

def clip_gradients(grad_mem, size, clip_value=0.5):
    """Clip gradients to prevent exploding gradients"""
    grad_array = np.zeros(size, dtype=np.float32)
    harness.copy_from_gpu(grad_mem, grad_array)
    norm = np.linalg.norm(grad_array)
    if norm > clip_value:
        grad_array = grad_array * clip_value / norm
    harness.copy_to_gpu(grad_array, grad_mem)
...
for epoch in range(1, EPOCHS + 1):
    ...
    for batch_idx, (real_imgs, _) in enumerate(train_loader):
        ...
        # --- Compute losses with proper label smoothing ---
        ...
        real_labels_smooth = apply_label_smoothing(real_labels, LABEL_SMOOTHING, is_real=True)
        fake_labels_smooth = apply_label_smoothing(fake_labels, LABEL_SMOOTHING, is_real=False)
        
        real_labels_gpu = to_gpu(real_labels_smooth)
        fake_labels_gpu = to_gpu(fake_labels_smooth)
        ...
        # BCE loss for G (use real labels without smoothing for generator)
        real_labels_gpu_G = to_gpu(real_labels)
        G_loss_gpu = BCE.forward(D_fake_gpu, real_labels_gpu_G, BATCH_SIZE)
        ...
        # --- Train Discriminator (less frequently and with gradient clipping) ---
        if batch_idx % 3 == 0:  # Train D even less frequently
            ...
            # Clip gradients
            clip_gradients(grad_real_gpu, BATCH_SIZE, GRADIENT_CLIP)
            clip_gradients(grad_fake_gpu, BATCH_SIZE, GRADIENT_CLIP)
        ...
        BCE.backward(D_fake_gpu, real_labels_gpu_G, grad_gan_gpu, BATCH_SIZE)
        
        # Clip gradients
        clip_gradients(grad_gan_gpu, BATCH_SIZE, GRADIENT_CLIP)
    ...
    if epoch % SAVE_EVERY == 0:
        ...
        plt.tight_layout()
        plt.savefig(f'samples_advanced/d_output_hist_epoch_{epoch}.png', dpi=150, bbox_inches='tight')
        plt.close()
...
plt.tight_layout()
plt.savefig('training_curves_advanced.png', dpi=150, bbox_inches='tight')
plt.close()
