...
from gan_networks_dcgan_full import build_generator, build_discriminator
...
LR_D = 2e-4  # Discriminator learning rate (same for balance)
BETA1 = 0.5
BETA2 = 0.999
EPSILON = 1e-8
SAVE_EVERY = 10
...
success = harness.compile_kernels()
print(f"Compilation success: {success}")
print(f"Library loaded: {harness.lib is not None}")
if harness.lib is None:
    print("ERROR: CUDA library not loaded!")
    exit(1)
...
# --- Training Loop ---
def save_images(samples, epoch, out_dir='samples_dcgan'):
...
for epoch in range(1, EPOCHS + 1):
    ...
    for batch_idx, (real_imgs, _) in enumerate(train_loader):
        ...
        # --- DCGAN Training Strategy ---
        # Train discriminator and generator with balanced approach
        d_real_mean = D_real.mean()
        d_fake_mean = D_fake.mean()
        
        # Train discriminator (with gradient penalty-like approach)
        if (d_real_mean < 0.9) and (d_fake_mean > 0.1):
    ...
    if epoch % SAVE_EVERY == 0:
        ...
        plt.tight_layout()
        plt.savefig(f'samples_dcgan/d_output_hist_epoch_{epoch}.png', dpi=150, bbox_inches='tight')
        plt.close()
...
axes[0, 0].set_title('DCGAN Training Losses')
...
plt.tight_layout()
plt.savefig('training_curves_dcgan.png', dpi=150, bbox_inches='tight')
plt.close()
