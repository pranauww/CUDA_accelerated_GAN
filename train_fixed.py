...
IMG_DIM = 28 * 28
EPOCHS = 100
LR_G = 2e-4  # Higher learning rate for generator
...
# --- Data Loading (MNIST, [0, 1] range for stability) ---
...
# --- Training Loop ---
def save_images(samples, epoch, out_dir='samples_fixed'):
    os.makedirs(out_dir, exist_ok=True)
    grid = samples.reshape(-1, 1, 28, 28)
    # samples are already in [0, 1] range
...
for epoch in range(1, EPOCHS + 1):
    ...
    for batch_idx, (real_imgs, _) in enumerate(train_loader):
        # --- Prepare real and fake data (keep in [0, 1] range) ---
        ...
        # --- Compute losses with NO label smoothing (simpler) ---
        ...
        # --- Train Discriminator (balanced frequency) ---
        if batch_idx % 2 == 0:  # Train D every other batch
    ...
    if epoch % SAVE_EVERY == 0:
        ...
        plt.tight_layout()
        plt.savefig(f'samples_fixed/d_output_hist_epoch_{epoch}.png', dpi=150, bbox_inches='tight')
        plt.close()
