...
LR_D = 1e-4  # Discriminator learning rate (slower)
BETA1 = 0.5
BETA2 = 0.999
EPSILON = 1e-8
SAVE_EVERY = 5
...
# --- Training Loop ---
def save_images(samples, epoch, out_dir='samples_improved'):
...
for epoch in range(1, EPOCHS + 1):
    ...
    for batch_idx, (real_imgs, _) in enumerate(train_loader):
        ...
        real_labels_smooth = apply_label_smoothing(real_labels, LABEL_SMOOTHING)
        fake_labels_smooth = apply_label_smoothing(fake_labels, LABEL_SMOOTHING)
        ...
        # BCE loss for G (use real labels for generator)
        ...
        if batch_idx % 2 == 0:  # Train D less frequently
    ...
    if epoch % SAVE_EVERY == 0:
        ...
        plt.tight_layout()
        plt.savefig(f'samples_improved/d_output_hist_epoch_{epoch}.png', dpi=150, bbox_inches='tight')
        plt.close()
